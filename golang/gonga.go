...
/*
	...
	current image (only of the master VM for the moment) to a
	dump file.
...
var dump = flag.String("d", "", "image dump file")
...
func main() {
	...
	if flag.NArg() == 1 {
		fmt.Fprint(os.Stderr, "( Gonga: loading ", flag.Arg(0), ") ");
		ngaro.LoadDump(flag.Arg(0), img);
	} else {
		img = retroImage
	}
