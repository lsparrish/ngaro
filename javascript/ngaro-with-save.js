...
function handleDevices()
{
  ...
  if (ports[4] == 1)
  {
    ...
    ports[0] = 1;
    var zip = new JSZip();
    zip.add("retroImage.csv", btoa(image), {base64: true});
    zip.add("build-image.php", "<? $raw = file_get_contents('retroImage.csv'); $mem = explode(',', $raw); $fp = fopen('retroImage', 'wb'); foreach($mem as $opcode) fwrite($fp, pack('i', (int)$opcode)); fclose($fp); ?>");
